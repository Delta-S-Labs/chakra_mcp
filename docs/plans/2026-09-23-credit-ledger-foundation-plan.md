...
1. Migration `0033_credits_expand.sql`, in this order (INSERT must follow CREATE):
   ...
   2. `CREATE TABLE credit_ledger (...)` + `CREATE INDEX credit_ledger_account_created_idx ON credit_ledger (account_id, created_at DESC);` (schema per spec).
   3. Backfill rate limit: `UPDATE accounts a SET rate_limit_per_min = p.rate_limit_per_min FROM plans p WHERE p.id = a.plan_id;`
   4. Seed balances + ledger + marker (single statement so they can't diverge):
      ```sql
      WITH seeded AS (
        UPDATE accounts
      ...
      INSERT INTO credit_ledger (id, account_id, delta_mc, reason, balance_after_mc)
      SELECT gen_random_uuid(), id, 100000, 'migration_seed', credit_balance_mc FROM seeded;
      ```
     (Confirm `gen_random_uuid()` is available — `pgcrypto`/PG13+; the repo already
     uses `uuid` PKs, verify at implementation.)
2. `SharedConfig`: add `hosting_mode: HostingMode` (`Managed`|`SelfHosted`, default
   ...
   `CREDITS_COST_PER_INVOCATION_MC` / `LIMITS_DEFAULT_RATE_PER_MIN`. Keep the
   literal `100000` in the migration and the config default in sync (comment both).
...
- `backend/relay/src/state.rs` (thread credit config: cost_per_invocation, default grant)
- `backend/relay/src/main.rs`, `backend/server/src/main.rs` (build state from config)
- consume sites: `forwarder.rs::persist_invocation`, `inbox_bridge.rs::park`,
  `handlers/invoke.rs` (`invoke_trusted`, `invoke_public`), `handlers/mcp.rs::invoke`
...
1. `credits.rs`:
   ...
   - `settle_free_grant(tx, account, default_grant_mc)` — the CTE from the spec:
     ...
     with `RETURNING`, gating the `free_grant` ledger INSERT; 12-month catch-up cap.
...
5. `state.rs` + mains: add credit config to `RelayState` (builder like
   `.with_limits_enforce`) sourced from `SharedConfig`.
6. `cargo sqlx prepare --workspace -- --tests` (new credit queries) → commit `.sqlx`.
...
- Four surfaces: port `successful_pull_increments_quota_counter` et al. to assert
  `credit_balance_mc` dropped by `cost_mc`; exhausted account → 429 with new codes,
  at A2A push/pull, `/v1/invoke` trusted+public, MCP.
...
should be equivalent (seed 100k mc = 1000 invocations = prior free quota).
